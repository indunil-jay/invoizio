...
export const createInvoiceUseCase = async (data: CreateInvoiceDTO) => {
    ...
    try {
        const results = await transactionManagerService.startTransaction(
            async (tx) => {
                try {
                    ...
                    const newClientAddressDocument =
                        await clientAddressRepository.insert(
                            {
                    ...
                    const newInvoiceDocument = await invoiceRepository.insert(
                        {
                            ...
                            totalBasePrice:
                                data.invoice.totalBasePrice.toPrecision(),
                            totalDiscount:
                                data.invoice.totalDiscount.toPrecision(),
        ...
        await emailService.sendInvoiceDetailsToClient(
